...
# 분석 파이프라인(run_analysis_for_tickers / fetch_sigma_enrichment / save_multi_timeframe)은
# app.stock.analysis_service 로 이관 — router(엔드포인트)와 scheduler 양쪽이 공유.
...
async def analyze(
    ...
    resp = await run_analysis_for_tickers(target_tickers, db, get_provider_router())
