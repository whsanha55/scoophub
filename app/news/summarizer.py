...
[{"idx": 1, "summary_ko": "...", "importance": 3, "title_ko": "..."}, ...]"""
...
class NewsSummarizer:
    ...
    async def summarize_pending(self) -> dict:
        ...
        rows = await self.db.fetch(
            "SELECT id, title, summary, importance FROM news_articles "
    ...
    async def _process_chunk(self, chunk: list) -> dict:
        ...
        await self.db.execute(
            ...
            "  summary_status = v.status, updated_at = NOW() "
            "FROM (SELECT * FROM unnest($1::int[], $2::text[], $3::text[], $4::smallint[], $5::text[]) "
            "      AS t(id, title, summary, importance, status)) AS v "
            "WHERE n.id = v.id",
            ids, titles, summaries, importances, statuses,
        )
